...
export async function query(params,data) {
  return request(`http://182.140.221.130:8080/yixian/sys/user/findListByLimitPage.do?${qs.stringify(params)}`, { mode: 'cors',  method: 'post', body: JSON.stringify(data), });
}
...
export async function update(params, data) {
  return request(`http://182.140.221.130:8080/yixian/sys/user/changeEntity.do?${qs.stringify(params)}`, { mode: 'cors', method: 'post', body: JSON.stringify(data), });
}
