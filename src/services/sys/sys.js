...
export async function query(params,data) {
  return request(`/yixian/sys/role/findListByLimitPage.do?${qs.stringify(params)}`, { mode: 'cors', method: 'post', body: JSON.stringify(data), });
}
...
export async function remove(params,data) {
  return request(`/yixian/sys/role/deleteEntity.do?${qs.stringify(params)}`, { mode: 'cors', method: 'post', body: JSON.stringify(data), });
}
